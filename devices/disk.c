...
/*
 * SEC_NO 섹터를 디스크 D에서 버퍼로 읽습니다.
 * 이 버퍼에는 DISK_SECTOR_SIZE 바이트를 위한 공간이 있어야 합니다.
 * 디스크에 대한 액세스를 내부적으로 동기화하므로 디스크별 외부 잠금 불필요
 */
...
/* Write sector SEC_NO to disk D from BUFFER, which must contain
   ...
   per-disk locking is unneeded. */
/*
 * BUFFER에서 디스크 D에 섹터 'SEC_NUMBER'를 씁니다.
 * 이 디스크 D에는 DISK_SECTOR_SIZE 바이트가 포함되어야 합니다.
 * 디스크가 데이터 수신을 승인한 후 반환합니다.
 * 디스크에 대한 액세스를 내부적으로 동기화하므로 디스크별 lock 불필요
 */
...
/* Resets an ATA channel and waits for any devices present on it
   to finish the reset. */
/* ATA 채널을 재설정하고 재설정이 완료될 때까지 기다립니다 */
static void
