...
// START와 START + CNT 사이의 B 비트가 VALUE로 설정되어 있으면 true를 반환하고, 그렇지 않으면 false를 반환
...
/* Finds and returns the starting index of the first group of CNT
   ...
   * 모두 VALUE로 설정된 START 이후의 'B'에서 CNT 연속 비트 그룹의 첫 번째 시작 인덱스를 찾아 반환
   * 해당 그룹이 없으면 BITMAP_ERROR를 반환
   */
size_t
...
/* Finds the first group of CNT consecutive bits in B at or after
   ...
   setting them.
   * 모두 VALUE로 설정된 START 이후의 'B'에서 CNT 연속 비트의 첫 번째 그룹을 찾아 모두 !VALUE로 플립 후, 그룹의 첫 번째 비트의 인덱스를 반환합니다.
   * 해당 그룹이 없으면 BITMAP_ERROR를 반환
   * CNT가 0이면 0을 반환
