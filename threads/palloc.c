...
/* Obtains and returns a group of PAGE_CNT contiguous free pages.
   ...
   * PAGE_CNT 연속 사용 가능한 페이지 그룹을 가져오고 반환합니다.
   * PAL_USER가 설정된 경우 페이지는 사용자 풀에서 가져오고 그렇지 않은 경우 커널 풀에서 가져옵니다.
   * PAL_ZERO가 FLATS에 설정된 경우 페이지는 0으로 채워집니다.
   * 사용 가능한 페이지가 너무 적으면 PAL_ASSERT가 FLags에 설정되어 있지 않으면 null 포인터를 반환합니다. 이 경우 커널 패닉이 발생합니다.
   */
void *
...
void palloc_free_multiple(void *pages, size_t page_cnt)
{
	...
	ASSERT(pg_ofs(pages) == 0);			// page offset이 0이라면
	if (pages == NULL || page_cnt == 0) // 페이지가 NULL이거나 page_cnt가 0이라면 종료
		return;
